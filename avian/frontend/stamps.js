...
/* ============================================================
   ...
   ============================================================ */
/* fx.js: real canvas image treatments for Vogel Bezoeken stamps.
...
(function () {
    ...
    html:
      '<div class="face field tpl-fieldguide" style="--face:#d7b46d">' +
        ...
        '<div class="fg-caption">' +
          ...
          '<div class="fg-meta"><b>VOGEL<br>BEZOEKEN</b><span class="fg-sci">{{SCI_STACK}}</span></div>' +
        '</div>' +
  ...
  if (geoIssue) geoIssue.html = geoIssue.html
    ...
    .replace('<div class="geo-spine">{{ORDER}}</div>', '<div class="geo-spine">{{ORDER}} · Vogel Bezoeken</div>')
    .replace('<span class="gproj">\u00a0· Vogel Bezoeken</span>', '');
    ...
    owlIssue.html =
      '<div class="face tpl-opart" style="--face:#fffef8">' +
        '<div class="ow-field">' +
          ...
          '<div class="ow-brand"><b>VOGEL BEZOEKEN</b><span>{{ORDER}}</span></div>' +
  ...
  // Display names are independent of the illustration design groups above.
  var DUTCH_GROUPS = {
    'Hummingbirds':'Kolibries', 'Crows & Jays':'Kraaien', 'Herons':'Reigers',
    'Waterfowl':'Eendachtigen', 'Owls':'Uilen', 'Hawks':'Roofvogels',
    'Gulls':'Meeuwen en verwanten', 'Sparrows':'Amerikaanse gorzen',
    'Finches':'Vinken', 'Doves & Pigeons':'Duiven', 'Thrushes':'Lijsters',
    'Flycatchers':'Tirannen', 'Mockingbirds & Thrashers':'Spotlijsters',
    'Waxwings':'Pestvogels', 'Blackbirds & Orioles':'Troepialen',
    'Chickadees & Titmice':'Mezen en verwanten', 'Warblers & Vireos':'Zangers en verwanten',
    'Kingfishers':'IJsvogels', 'Shorebirds':'Strandlopers en snippen',
    'Swallows':'Zwaluwen', 'Treecreepers':'Boomkruipers'
  };
  var DUTCH_FAMILIES = {
    Passer:'Mussen', Sturnus:'Spreeuwen', Tyto:'Kerkuilen',
    Parus:'Mezen', Cyanistes:'Mezen', Poecile:'Mezen', Baeolophus:'Mezen',
    Erithacus:'Vliegenvangers', Ficedula:'Vliegenvangers', Muscicapa:'Vliegenvangers',
    Pica:'Kraaien', Dendrocopos:'Spechten', Dryobates:'Spechten',
    Picoides:'Spechten', Colaptes:'Spechten', Melanerpes:'Spechten',
    Sitta:'Boomklevers', Troglodytes:'Winterkoningen', Thryomanes:'Winterkoningen',
    Regulus:'Goudhanen', Corthylio:'Goudhanen', Psaltriparus:'Staartmezen',
    Falco:'Valken', Pandion:'Visarenden', Phalacrocorax:'Aalscholvers',
    Nannopterum:'Aalscholvers', Pelecanus:'Pelikanen',
    Fringilla:'Vinken', Chloris:'Vinken'
  };
  function familyOf(sci) {
    return DUTCH_FAMILIES[String(sci || '').split(' ')[0]]
      || DUTCH_GROUPS[groupFor(sci)] || 'Overige vogels';
  }
  ...
  function markup(bird, cutoutUrl, template) {
    ...
    if (!t) return '';
    var box = boxFor(t.ar);
    var fam = bird.family || familyOf(bird.sci), lat = familyOf(bird.sci);
    ...
    /* Gulls and Columbidae are flight-composed issues.  They must always use
       ...
       cutout endpoint. */
    var primarySource = !placeholderArt && groupFor(bird.sci) === 'Doves & Pigeons'
    ...
    var html = t.html
      ...
      .replace(/\{\{PROJECT_STACK\}\}/g, stackProject('Vogel Bezoeken'))
      .replace(/\{\{PROJECT\}\}/g, 'Vogel Bezoeken');
    ...
    var issue = '<div class="stamp-fit"' + placeholderAttr + ' data-family="' + esc(groupFor(bird.sci) || 'Other') + '" data-common="' + esc(commonName) + '" style="width:' + box.w + 'px;height:' + box.h + 'px">' +
           ...
           '<figure class="stamp"' + placeholderAttr + ' data-perf="' + perf + '" data-style="' + t.id + '" data-family="' + esc(groupFor(bird.sci) || 'Other') + '" data-common="' + esc(commonName) + '"' +
